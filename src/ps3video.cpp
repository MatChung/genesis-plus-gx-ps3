...
{
   	LOG_DBG("PS3Graphics constructor\n");
	m_smooth = true;
	m_pal60Hz = false;
	m_overscan = false;
...
{
   	LOG_DBG("PS3Graphics destructor\n");
	Deinit();
}

void PS3Graphics::Deinit()
{
   	LOG_DBG("PS3Graphics::Deinit()\n");
	PSGLGraphics::Deinit();
	if(vertex_buf)
	{
		free(vertex_buf);
...
{
   	//LOG_DBG("PS3Graphics::FlushDbgFont()\n");
	cellDbgFontDraw();
}

void PS3Graphics::Swap()
{
   	//LOG_DBG("PS3Graphics::Swap()\n");
	psglSwap();
}
...
{
   	//LOG_DBG("PS3Graphics::Clear()\n");
	glClear(GL_COLOR_BUFFER_BIT);
}

void PS3Graphics::SetViewports()
{
   LOG_DBG("PS3Graphics::SetViewPorts()\n");
...
{
   	LOG_DBG("PS3Graphics::SetOverscan(%d, %f)()\n", will_overscan, amount);
...
{
	LOG_DBG("PS3Graphics::SetPAL60Hz(%d)\n", pal60Hz);
	m_pal60Hz = pal60Hz;
}

bool PS3Graphics::GetPAL60Hz()
{
	LOG_DBG("PS3Graphics::GetPAL60Hz()\n");
	return m_pal60Hz;
}

void PS3Graphics::SetAspectRatio(bool keep_aspect)
{
	LOG_DBG("PS3Graphics::SetAspectRatio(%d)\n", keep_aspect);
	if (keep_aspect)
	{
...
{
	LOG_DBG("PS3Graphics::ChangeResolution(%d)\n", resId);
	int32_t ret;
...
{
	//LOG_DBG("PS3Graphics::UpdateCgParams(%d, %d, %d, %d)\n", width, height, tex_width, tex_height);
...
{
	LOG_DBG("PS3Graphics::InitScreenQuad(%d, %d)\n", width, height);
	screenQuad.v1.x = 0;
	screenQuad.v1.y = 0;
	screenQuad.v1.z = 0;
...
{
	LOG_DBG("PS3Graphics::SetSmooth(%d)\n", smooth);
	m_smooth = smooth;
...
{
	LOG_DBG("PS3Graphics::CheckCgError(%d)\n",line);
...
{
	LOG_DBG("PS3Graphics::LoadShaderFromFile()\n");
...
{
	LOG_DBG("PS3Graphics::LoadShaderFromSource()\n");
...
{
	LOG_DBG("PS3Graphics::InitCg()\n");
	
	cgRTCgcInit();

	LOG_DBG("PS3Graphics::InitCg() - About to create CgContext\n");
...
{
   LOG_DBG("PS3Graphics::LoadFragmentShader(%s)\n", shaderPath.c_str());
...
int32_t PS3Graphics::PSGLInit()
{
	...
	if (ret != CELL_OK)
	{
		LOG_DBG("Failed to InitCg: %d", __LINE__);
	}
